...
export default {
    ...
    props: ['deliverySheets', 'trucks', 'warehouses', 'routes', 'users', 'allDrivers', 'allTrucks', 'settings'],
    data() {
        return {
            ...
            deleteModal: null,
            delivery_sheets_data: this.deliverySheets,
            search: '',
            selectedStatus: null,
            statusOptions: [
                { id: 1, name: 'Draft' },
    ...
    mounted() {
        ...
        const $targetEl = document.getElementById('add-deliverySheet-modal');
        this.addLoadSheetModal = new Modal($targetEl);

        const $targetElUpload = document.getElementById('upload-excel-deliverySheet');
    ...
    methods: {
        ...
        deleteLoadsheet() {
            this.$inertia.delete(`/company/deliverySheets/${this.selectedLoadSheet.id}`, {
                preserveScroll: true,
                onSuccess: () => {
        ...
        selectLoadSheet(event, deliverySheet) {
            this.selectedLoadSheet = deliverySheet;
            this.addLoadSheetModal.show();
        },
        confirmDeleteDialog(event, deliverySheet) {
            this.selectedLoadSheet = deliverySheet;
            this.deleteModal.show();
        },
        openSingleProduct(deliverySheet) {
            this.$inertia.visit(`/company/deliveries/${deliverySheet.id}`)
        },


        submitSearch() {
            this.isLoading = true;
            axios.post(`/company/filter-delivery-sheets`, {
                ...
                .then((response) => {
                    ...
                    this.delivery_sheets_data = response.data;
    ...
    watch: {
        deliverySheets: {
            handler(newValue) {
                this.delivery_sheets_data = newValue;
            },
            deep: true
        }
    }
    ...
    <MainLayout>
        ...
        <div
            ...
            <div class="w-full mb-1">
                <div class="mb-4">
                    <BreadCrumb :title="'Delivery Sheets'" />
                    <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Delivery Sheets</h1>
                </div>
                <div class="sm:flex">
                    <div
                        ...
                        <span
                            ...
                            {{ delivery_sheets_data.meta.total }} records found</span>
                    </div>
                    ...
                    <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
                        ...
                        <button type="button" @click="openAddDeliverySheetModal()"
            ...
            <div class="overflow-x-auto">
                <div class="inline-block min-w-full align-middle">
                    ...
                    <div class="overflow-hidden shadow">
                        <TableLayout :isLoading="isLoading" :hasData="delivery_sheets_data.data.length > 0 ? true : false">
                                ...
                                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                                        ...
                                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700"
                                            v-for="deliverySheet in delivery_sheets_data.data">
                                            <td class="w-4 p-4">
                                                <div class="flex items-center">
                                            ...
                                            <td class="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap">
                                                ...
                                                <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                                    <div class="text-base font-semibold text-gray-900 dark:text-white">
                                                     {{ deliverySheet.driver.first_name }} {{
                                                            deliverySheet.driver.last_name
                                                        }}</div>
                                                    ...
                                                    <div
                                                        ...
                                                        <CopyButton @click="copyToClipboard(deliverySheet.id)" />
                                                    </div>
                                                </div>
                                            </td>
                                            <td
                                                ...
                                                <div class="flex-coulumn">
                                                    <h1 class="text-base font-semibold">
                                                        {{ deliverySheet.truck.make_model }}</h1>
                                                    ...
                                                    <div
                                                        ...
                                                        <CopyButton
                                                            @click="copyToClipboard(deliverySheet.loadsheet_number)" />
                                                    </div>
                                            ...
                                            <!-- <td
                                                class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                <h1 class="text-base font-semibold">
                                                    {{ deliverySheet.warehouse.name }}</h1>

                                                <div class="mt-1 flex text-sm font-normal text-gray-500 dark:text-gray-400">
                                                    <span class="text-xs font-medium text-gray-500">
                                                        {{ deliverySheet.warehouse.code }}
                                                    </span>

                                                    <CopyButton @click="copyToClipboard(deliverySheet.warehouse.code)" />
                                                </div>
                                            </td> -->
                                            <td
                                                class=" p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {{ deliverySheet.route.name }}
                                                <div class="mt-1 flex text-sm font-normal text-gray-500 dark:text-gray-400">
                                                    <span class="text-xs font-medium text-gray-500">
                                                        Centers: {{ deliverySheet.route.number_centers }}
                                                    </span>
                                            ...
                                            <td
                                                class=" p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                <span :class="getStatusBackGroundColor(deliverySheet.status)"
                                                    class="bg-green-500 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">
                                                    {{ deliverySheet.status }}
                                                </span>
                                            </td>

                                            <td
                                                class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {{ formatDateTime(deliverySheet.start_date_time) }}
                                            </td>

                                            <td class="p-4 space-x-2 whitespace-nowrap">
                                                <TableActionButtons :type="'deliverySheet'" :has_view="true"
                                                    @delete="confirmDeleteDialog($event, deliverySheet)"
                                                    @edit="selectLoadSheet($event, deliverySheet)"
                                                    @view="openSingleProduct(deliverySheet)" />
                                            </td>
                                        </tr>
                            ...
                            <template v-slot:action-button>
                                <button @click="openAddDeliverySheetModal()"
                                    ...
                                    <span>Create DeliverySheet</span>
                                </button>
                            </template>
                        </TableLayout>
                    </div>
        ...
        <div v-if="delivery_sheets_data.data.length > 0"
            class="sticky bottom-0 right-0 items-center w-full p-4 bg-white border-t border-gray-200 sm:flex sm:justify-between dark:bg-gray-800 dark:border-gray-700">
            <Pagination :from="delivery_sheets_data.meta.from" :to="delivery_sheets_data.meta.to"
                :total="delivery_sheets_data.meta.total" :next_page_url="delivery_sheets_data.links.next"
                :prev_page_url="delivery_sheets_data.links.prev" />
        </div>

        <AddDeliverySheetModal :settings="settings" :deliverySheet="selectedLoadSheet" :trucks="trucks"
            :warehouses="warehouses" :routes="routes" :users="users" @save="closeAddLoadSheetModal()" />
        <ConfirmDeleteDialog @cancel="closeDeleteModal" @yes="deleteLoadsheet" :type="'deliverySheet'" />
    </MainLayout>
</template>
