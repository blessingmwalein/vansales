                    ...
                    <form action="#">
                        ...
                        <div>
                            ...
                            <multiselect
                                ...
                                v-model="form.user_id" :options="usersOptions" label="name" track-by="id">
                            </multiselect>
                        ...
                        <div>
                            ...
                            <multiselect
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white   dark:border-gray-600"
                                v-model="form.warehouse_id" :options="warehousesOptions" label="name" track-by="id">
                            </multiselect>
...
export default {
    ...
    mounted() {
        ...
        this.warehousesOptions = this.warehouses.map(warehouse => {
            return {
                id: warehouse.id,
        ...
        if (this.branch) {
            this.form.user_id = this.usersOptions.find(route => route.id == this.branch.user_id);
        }
    },
    data() {
        return {
            form: useForm({
                ...
                user_id: this.branch?.user_id || null,
    ...
    methods: {
        ...
        create() {
            this.form.user_id = this.form.user_id ? this.form.user_id.id : null;
            this.form.warehouse_id = this.form.warehouse_id ? this.form.warehouse_id.id : null;
            this.form.post('/company/branches', {
                preserveScroll: true,
                onSuccess: () => {
                    this.$emit('save');
                }
        ...
        update() {
            this.form.user_id = this.form.user_id ? this.form.user_id.id : null;
            this.form.warehouse_id = this.form.warehouse_id ? this.form.warehouse_id.id : null;
            this.form.put(`/company/branches/${this.branch.id}`, {
                preserveScroll: true,
                onSuccess: () => {
                    this.$emit('save');
                }
    ...
    watch: {
        // Watch the 'branch' prop for changes
        branch: {
            handler(newValue) {
                // Update the 'form' data when 'branch' prop changes
                ...
                this.form.user_id = this.usersOptions.find(route => route.id == newValue.user_id);
                this.form.warehouse_id = this.warehousesOptions.find(warehouse => warehouse.id == newValue.warehouse_id )
