<template>
    <div id="add-delivery-modal" tabindex="-1"
                    ...
                    <form class="" action="#">
                        <div>
                            <label for="description"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Search Customer By
                                name, email,
                                phone</label>
                        ...
                        <div class="mt-2">
                            <label for="description"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Items(Search By
                                Description
                                or Code)</label>
                            <VueMultiselect :searchable="true" :options="productOptions" :custom-label="codeWithName"
                                :clearOnSelect="true" @search-change="searchProducts" @select="selectProduct">
                            </VueMultiselect>
...
export default {
    ...
    data() {
        return {
            ...
            selectedProducts: [],
            form: useForm({
                delivery_sheet_id: this.deliverySheet?.id || null,
                customer_id: null,
                items: [],
            })
        }
    ...
    methods: {
        searchProducts(value, loading) {
            axios
                .post(`/company/products-search-warehouse`, {
                    id: this.deliverySheet.warehouse.id,
                    search: value
                })
                .then((response) => {
                    this.productOptions = response.data.data.map((stock) => {
                        console.log(stock)
                        return {
        ...
        searchCustomer(value, loading) {
            axios
                ...
                .then((response) => {
                    ...
                    this.options = response.data?.data?.map((customer) => {
        ...
        submit() {
            ...
            this.form.items = this.selectedProducts.map((product) => {
                return {
                    stock_id: product.stock_id,
                    quantity: product.quantity
                };
            });
            this.form.post('/company/add-delivery', {
                preserveScroll: true,
                onSuccess: () => {
    ...
    watch: {
        ...
        deliverySheet: {
            handler(newValue) {
                this.form.delivery_sheet_id = newValue?.id || null;
            },
